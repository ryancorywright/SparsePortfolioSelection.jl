...
######################################################################################################################################################
#### Note: you will need to manually change the miop_formulation file to set a time limit of 120s before running this code, to reproduce output.
######################################################################################################################################################
...
for portfolioNumber=2:4
     ...
     open("results/Table5ResultsOutput.csv","a") do fp
     println(fp, "Instance, cardinality, regularizer, method, solvetime, gap, nodecount, iters")
     println(fp, "beasley"*string(portfolioNumber), ",", thePortfolioData.k, ",",
                       "100/sqrt(n)", ",", "OA", ",",  sparse_Portfolio.solveTime, ",", sparse_Portfolio.boundGap, ",", sparse_Portfolio.nodeCount, ",", sparse_Portfolio.numIters)
     end
     # You will need to manually check there are 50 (not 200) cuts applied here.
     ...
     println("Return is:", sparse_Portfolio.expectedReturn)
     println("Reg term is:", 0.5*sum(sparse_Portfolio.w[i]^2/thePortfolioData.γ[sparse_Portfolio.indices[i]] for i=1:size(sparse_Portfolio.indices,1)))
     open("results/Table5ResultsOutput_b.csv","a") do fp
     println(fp, "Instance, cardinality, regularizer, method, solvetime, gap, nodecount, iters")
     println(fp, "beasley"*string(portfolioNumber), ",", thePortfolioData.k, ",",
                       "100/sqrt(n)", ",", "OA-Kelley (50 cuts)", ",",  sparse_Portfolio.solveTime, ",", sparse_Portfolio.boundGap, ",", sparse_Portfolio.nodeCount, ",", sparse_Portfolio.numIters)
     end

     #Test against Big-M for correctness
     sparse_Portfolio_Cplex_bigM=cplex_raw_bigM(thePortfolioData, 120.0, 1e-8, 1)
     ...
     open("results/Table5ResultsOutput.csv","a") do fp
     println(fp, "Instance, cardinality, regularizer, method, solvetime, gap, nodecount")
     println(fp, "beasley"*string(portfolioNumber), ",", thePortfolioData.k, ",",
                     "100/sqrt(n)", ",", "CPLEX Big-M", ",",  sparse_Portfolio_Cplex_bigM.solveTime, ",", sparse_Portfolio_Cplex_bigM.boundGap, ",", sparse_Portfolio_Cplex_bigM.nodeCount)
    end

    # #  # Test against MISOCP for correctness
    sparse_Portfolio_Cplex=cplex_raw_MISOCP(thePortfolioData, 120.0, 1e-8, 1)
    ...
    println(fp, "Instance, cardinality, regularizer, method, solvetime, gap, nodecount")
    println(fp, "beasley"*string(portfolioNumber), ",", thePortfolioData.k, ",",
                        "100/sqrt(n)", ",", "CPLEX MISOCP", ",",  sparse_Portfolio_Cplex.solveTime, ",", sparse_Portfolio_Cplex.boundGap, ",", sparse_Portfolio_Cplex.nodeCount)
    end
end
