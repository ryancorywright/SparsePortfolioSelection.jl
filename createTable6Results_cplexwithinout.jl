...
for portfolioNumber in 5:10

    for theK in [6, 8, 10, 12, 400]
     thePath=pwd()
     pathName = thePath*"/data/MV/size"
     pathName2 = thePath*"/data/diagonals/s/"
     pathName3 = thePath*"/data/diagonals/s/"
     thePortfolioData=getProblemDataFG_minreturn(pathName,pathName2, pathName3, 400, :Pard, portfolioNumber, theK)


    sparse_Portfolio_Cplex=cplex_raw_MISOCP_inout(thePortfolioData, 600.0, 1e-8, portfolioNumber)
    ...
    open("results/Table6ResultsOutput_cplexinout.csv","a") do fp
    ...
    println(fp, "pard400_"*string(portfolioNumber), ",", thePortfolioData.k, ",",
                        "1000/n", ",", "CPLEX MISOCP", ",",  sparse_Portfolio_Cplex.solveTime, ",", sparse_Portfolio_Cplex.nodeCount)
        end
    end
end
