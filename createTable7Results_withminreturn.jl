include("src/core.jl")
using JLD
function createSP500Data(rank, theK)
    sp500Data=load("data/SP500Rank"*string(rank)*".jld", "SP500Rank"*string(rank))


    n=size(sp500Data.X,2)
    μ=sp500Data.μ*2769/(11*12)
    X=sp500Data.X*sqrt((2769/(11*12)))
    Σ= X'*X

    min_investment=zeros(n)

    γ=(100.0/sqrt(n))*ones(n) #you need to change this line if you want to reproduce results with γ=1/sqrt(n)
...
for theK in [10, 50, 100, 200]
        ...
        thePortfolioData=createSP500Data(rank, theK)
        ...
        open("results/Table7ResultsOutput_sp500_withminreturn.csv","a") do fp
